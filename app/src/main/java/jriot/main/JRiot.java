...
import com.softstew.lollookup.util.ChampionNameHelper;
...
public class JRiot {
	...
	public JRiot(String key, String region, Context context) {
		super();
		connectTimeout = Integer.parseInt(PreferenceManager
				.getDefaultSharedPreferences(context).getString(
						"connection_connecttimeout", "20"));
		readTimeout = Integer.parseInt(PreferenceManager
				.getDefaultSharedPreferences(context).getString(
    ...
    public CurrentGameInfo getPlayersCurrentGame(String platformId, long summonerId) throws JRiotException{
        ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
	...
	public ChampionList getChampions() throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl() + "/v1.2/champion"
				+ "?api_key=" + apiKey);
	...
	public ChampionList getFreeChampions() throws JRiotException {
		...
		ChampionList championList = gson.fromJson(response, ChampionList.class);
		for (Champion champ : championList.getChampionList()) {
			champ.setName(ChampionNameHelper.getName(caller, gson, apiKey,
					champ.getId()));
		}
		return championList;
	}
	...
	public Champion getChampion(int id) throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl() + "/v1.2/champion/"
				+ id + "?freeToPlay=true&api_key=" + apiKey);
	...
	public RecentGames getRecentGames(long summonerId) throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl()
	...
	public League getChallenger(String queue) throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl()
				+ "/v2.5/league/challenger" + "?type=" + queue + "&api_key="
				+ apiKey);
	 ...
	 * @throws JRiotException
	 */
	public Map<String, List<League>> getLeagueEntries(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		...
		String response = caller.request(generateBaseUrl()
				+ "/v2.5/league/by-summoner/" + ids + "/entry" + "?api_key="
				+ apiKey);
		Map<String, List<League>> leagueEntries = gson.fromJson(response,
				new TypeToken<Map<String, List<League>>>() {
				}.getType());
	 ...
	 * @throws JRiotException
	 */
	public List<League> getLeagueEntriesByTeam(String teamId)
			throws JRiotException {
	 ...
	 * @throws JRiotException
	 */
	public Map<String, List<League>> getLeagueEntriesByTeam(List<String> teamIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		...
		String response = caller.request(generateBaseUrl()
				+ "/v2.5/league/by-team/" + ids + "/entry" + "?api_key="
				+ apiKey);
		Map<String, List<League>> leagueEntries = gson.fromJson(response,
				new TypeToken<Map<String, List<League>>>() {
				}.getType());
	 ...
	 */
	public Map<String, List<League>> getLeagues(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl()
				+ "/v2.5/league/by-summoner/" + ids + "?api_key=" + apiKey);
		Map<String, List<League>> leagues = gson.fromJson(response,
				new TypeToken<Map<String, List<League>>>() {
				}.getType());
		return leagues;
	}
	 ...
	 * @throws jriot.main.JRiotException
	 */
	public Map<String, List<League>> getLeaguesByTeams(List<String> teamIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (String i : teamIds) {
		...
		String response = caller.request(generateBaseUrl()
				+ "/v2.5/league/by-team/" + ids + "?api_key=" + apiKey);
		Map<String, List<League>> leagues = gson.fromJson(response,
				new TypeToken<Map<String, List<League>>>() {
				}.getType());
		return leagues;
	 ...
	 *            Id of a summoner.
	 * @param season
	 *            Specifies the season you want the stats about (currently 3 or
	 *            4)
	 * @return PlayerStatsSummaryList that contains the stats
	 * @throws JRiotException
	 */
	public PlayerStatsSummaryList getPlayerStatsSummaryList(long summonerId,
			int season) throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl()
				+ "/v1.3/stats/by-summoner/" + summonerId + "/summary"
				+ "?season=SEASON" + season + "&api_key=" + apiKey);
		PlayerStatsSummaryList playerStatsSummaryList = gson.fromJson(response,
	...
	public RankedStats getRankedStats(long summonerId, int season)
		...
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String response = caller.request(generateBaseUrl()
				+ "/v1.3/stats/by-summoner/" + summonerId + "/ranked"
				+ "?season=SEASON" + season + "&api_key=" + apiKey);
	 ...
	 * @throws JRiotException
	 */
	public Map<String, MasteryPages> getMasteryPages(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl() + "/v1.4/summoner/"
				+ ids + "/masteries?api_key=" + apiKey);
		Map<String, MasteryPages> masteryPages = gson.fromJson(response,
				new TypeToken<Map<String, MasteryPages>>() {
				}.getType());
		return masteryPages;
	}
	 ...
	 * @throws JRiotException
	 */
	public Map<String, RunePages> getRunePages(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl() + "/v1.4/summoner/"
				+ ids + "/runes?api_key=" + apiKey);
		Map<String, RunePages> masteryPages = gson.fromJson(response,
				new TypeToken<Map<String, RunePages>>() {
				}.getType());
		return masteryPages;
	}
	 ...
	 * @throws JRiotException
	 */
	public Map<String, Summoner> getSummonersById(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl() + "/v1.4/summoner/"
				+ ids + "?api_key=" + apiKey);
		Map<String, Summoner> summoner = gson.fromJson(response,
				new TypeToken<Map<String, Summoner>>() {
				}.getType());
		return summoner;
	}
	 ...
	 * @throws JRiotException
	 */
	public Map<String, Summoner> getSummonersByName(List<String> summonerNames)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String names = "";
		...
		Gson gson = new GsonBuilder().setDateFormat("MM/dd/yyy HH:mm").create();
		String response = caller.request(generateBaseUrl()
				+ "/v1.4/summoner/by-name/" + names + "?api_key=" + apiKey);
		Map<String, Summoner> summoner = gson.fromJson(response,
				new TypeToken<Map<String, Summoner>>() {
				}.getType());
		return summoner;
	 ...
	 * @throws JRiotException
	 */
	public Map<String, String> getSummonerNames(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl() + "/v1.4/summoner/"
				+ ids + "/name?api_key=" + apiKey);
		Map<String, String> summonerNameMap = gson.fromJson(response,
	 ...
	 * @throws JRiotException
	 */
	public Map<String, List<Team>> getTeamsBySummoner(List<Long> summonerIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		String ids = "";
		for (long i : summonerIds) {
		...
		String response = caller.request(generateBaseUrl()
				+ "/v2.4/team/by-summoner/" + ids + "?api_key=" + apiKey);
		Map<String, List<Team>> teams = gson.fromJson(response,
				new TypeToken<Map<String, List<Team>>>() {
				}.getType());
		return teams;
	}
	 ...
	 * @throws JRiotException
	 */
	public Map<String, Team> getTeams(List<String> teamIds)
			throws JRiotException {
		ApiCaller caller = new ApiCaller(connectTimeout, readTimeout);
		...
		String response = caller.request(generateBaseUrl() + "/v2.4/team/"
				+ builder.toString() + "?api_key=" + apiKey);

		Map<String, Team> teams = gson.fromJson(response,
				new TypeToken<Map<String, Team>>() {
				}.getType());
		return teams;
	}
